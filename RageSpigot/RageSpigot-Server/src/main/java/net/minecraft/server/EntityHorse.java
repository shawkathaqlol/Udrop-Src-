...
public class EntityHorse extends EntityAnimal implements IInventoryListener {
    ...
    public void g(float f, float f1) {
        if (this.passenger != null && this.passenger instanceof EntityLiving && this.cG()) {
            ...
            if (this.br > 0.0F && !this.cv() && this.onGround) {
                ...
                this.ai = true;
                if (f1 > 0.0F) {
                    float f2 = (float) FastMath.sin(this.yaw * 3.1415927F / 180.0F);
            ...
            float f4 = (float) FastMath.sqrt(d0 * d0 + d1 * d1) * 4.0F;
    ...
    public void al() {
        super.al();
        if (this.bK > 0.0F) {
            float f = (float) FastMath.sin(this.aI * 3.1415927F / 180.0F);
