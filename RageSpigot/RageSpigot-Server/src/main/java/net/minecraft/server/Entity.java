...
import gg.ragemc.spigot.util.OptimizedRemoveUtil;
...
public abstract class Entity implements ICommandListener, OptimizedRemoveUtil.Marker {
	...
	public void K() {
		...
		if (this.world.isClientSide) {
			...
			if (this.fireProof) {
				...
				if (this.fireTicks % DedicatedServer.TPS == 0) { // RageSpigot - change default to TPS instead of 20
	...
	public void move(double d0, double d1, double d2) {
		...
		if (this.noclip) {
			...
			if (this.s_() && !flag && this.vehicle == null) {
				...
				this.M = (float) ((double) this.M + (double) FastMath.sqrt(d21 * d21 + d23 * d23) * 0.6D);
				this.N = (float) ((double) this.N + (double) FastMath.sqrt(d21 * d21 + d22 * d22 + d23 * d23) * 0.6D);
				if (this.N > (float) this.h && block.getMaterial() != Material.AIR) {
					this.h = (int) this.N + 1;
					if (this.V()) {
						float f = (float) FastMath.sqrt(this.motX * this.motX * 0.20000000298023224D + this.motY * this.motY + this.motZ * this.motZ * 0.20000000298023224D) * 0.35F;
	...
	public double f(double d0, double d1, double d2) {
		...
		return (double) FastMath.sqrt(d3 * d3 + d4 * d4 + d5 * d5);
	}
